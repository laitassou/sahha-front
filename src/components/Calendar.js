...
export default function DnDOutsideResource({ localizer, data }) {
  ...
  while (i < data.length) {
    ...
    //alert(el);
    var ev = {"id": el.id, "title": el.description, "start": new Date(el.start_time), "end": new Date(el.end_time),  "isDraggable": false,}
    if (events.find(obj => {return obj.id === ev.id && obj.title === ev.title}))
    {
      i++;
    }
    else 
